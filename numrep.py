...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='tell me a number and i will tell you a story')
    ...
    pure_number = None
    if values.binary is not None:
        pure_number = convert_to_decimal(values.binary, 2)
    elif values.octal is not None:
        pure_number = convert_to_decimal(values.octal, 8)
    elif values.decimal is not None:
        pure_number = values.decimal
    elif values.hex is not None:
        pure_number = convert_to_decimal(values.hex, 16)
    elif values.char is not None:
        pure_number = ord(values.char)
    else:
